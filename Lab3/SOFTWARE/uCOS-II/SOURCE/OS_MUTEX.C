...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    OS_ENTER_CRITICAL();	
	/* Is Mutex available? If mutex is available, then acquire and just return   */
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) 
	{
		...
		if (ptcb->OSTCBPrio != pip && pip < OSTCBCur->OSTCBPrio)   /*     Need to promote prio of owner?*/
		{ 
	...
	/*  The mutex is used by the other task   */
