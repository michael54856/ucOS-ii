...
void  OSIntExit (void)
{
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
			...
			while (ptcb->OSTCBExtPtr && ptcb->OSTCBPrio != OS_IDLE_PRIO)         /* Go through all TCBs in TCB list          */
			{ 

				if(((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline < earilestDeadline && currentTime >= ((TASK_INFO*)(ptcb->OSTCBExtPtr))->startTime)                  //(currentTime >= ptcb->startTime) means this task is not finish
				{
					earilestDeadline = ((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline;
					choosePrio = ptcb->OSTCBPrio;
				}
				else if(((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline == earilestDeadline && currentTime >= ((TASK_INFO*)(ptcb->OSTCBExtPtr))->startTime)            //(currentTime >= ptcb->startTime) means this task is not finish
				{
					if(ptcb == OSTCBCur)
					{
						earilestDeadline = ((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline;
...
void  OSStart (void)
{
    ...
    if (OSRunning == FALSE) 
	{
		...
		while (ptcb->OSTCBExtPtr && ptcb->OSTCBPrio != OS_IDLE_PRIO)         /* Go through all TCBs in TCB list          */
		{  
			if(((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline < earilestDeadline)
			{
				earilestDeadline = ((TASK_INFO*)(ptcb->OSTCBExtPtr))->deadline;
				choosePrio = ptcb->OSTCBPrio;
			}
